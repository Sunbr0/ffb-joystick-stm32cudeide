...
typedef struct _USBD_CUSTOM_HID_Itf
{
  ...
  int8_t (* Init)(void);
  int8_t (* DeInit)(void);
  int8_t (* OutEvent)(uint8_t event_idx, uint8_t state, USBD_HandleTypeDef *pdev);
#ifdef USBD_CUSTOMHID_CTRL_REQ_COMPLETE_CALLBACK_ENABLED
  int8_t (* CtrlReqComplete)(uint8_t request, uint16_t wLength, USBD_HandleTypeDef *pdev);
#endif /* USBD_CUSTOMHID_CTRL_REQ_COMPLETE_CALLBACK_ENABLED */
#ifdef USBD_CUSTOMHID_CTRL_REQ_GET_REPORT_ENABLED
  uint8_t *(* GetReport)(USBD_SetupReqTypedef *req, uint16_t *ReportLength, USBD_HandleTypeDef *pdev); // Dynamic GetReport
